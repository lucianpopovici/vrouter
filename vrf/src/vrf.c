...
static uint32_t fnv1a_raw(const uint8_t *d, size_t l, uint32_t n)
{
...
{ return fnv1a_raw((const uint8_t*)&val, sizeof(val), n); }
...
#define MIX(data,len) do { \
    for (size_t _i=0;_i<(len);_i++) { h^=((const uint8_t*)(data))[_i]; h*=FNV_PRIME; } \
} while(0)
...
static const char *jstr(const char *line,const char *key,char *out,size_t olen)
{
    ...
    const char *p=strstr(line,s); if (!p) return NULL;
...
int vrf_load_config(vrf_ctx_t *ctx, const char *path)
{
    ...
    while (fgets(line,sizeof(line),f)) {
        ...
        if (strcmp(type_buf,"vrf")==0) {
            ...
            jstr(line,"ifname",ifname,sizeof(ifname));
