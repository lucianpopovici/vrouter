                   ...
                   const char *req, char *resp, size_t rsz)
{
    ...
    /* ── port add ───────────────────────────────────────────── */
        ...
        jget(req,"port",name,sizeof(name));
        jget(req,"cost",cost_s,sizeof(cost_s));
        jget(req,"num",num_s,sizeof(num_s));
    ...
    /* ── port up / down ─────────────────────────────────────── */
        ...
        char name[16]={0}; jget(req,"port",name,sizeof(name));
    ...
    /* ── port config (edge, p2p, cost) ──────────────────────── */
        ...
        jget(req,"port",name,sizeof(name));
        jget(req,"edge",edge_s,sizeof(edge_s));
        jget(req,"p2p",p2p_s,sizeof(p2p_s));
        jget(req,"cost",cost_s,sizeof(cost_s));
        stp_port_t *p=stp_port_find(br,name);
        if(!p){snprintf(resp,rsz,"{\"status\":\"error\",\"msg\":\"port not found\"}");return;}
    ...
    /* ── receive BPDU (inject from test/other daemon) ────────── */
        ...
        jget(req,"port",port_s,sizeof(port_s));
        jget(req,"root",root_s,sizeof(root_s));
        jget(req,"bridge",bridge_s,sizeof(bridge_s));
        jget(req,"cost",cost_s,sizeof(cost_s));
    ...
    /* ── tick (advance timers) ───────────────────────────────── */
        ...
        char n_s[8]={0}; jget(req,"n",n_s,sizeof(n_s));
    ...
    /* ── set bridge priority ─────────────────────────────────── */
        ...
        char v[16]={0}; jget(req,"priority",v,sizeof(v));
    ...
    /* ── set mode (stp/rstp/mst) ─────────────────────────────── */
        ...
        char v[8]={0}; jget(req,"mode",v,sizeof(v));
    ...
    /* ── MST: map vlan → instance ────────────────────────────── */
        ...
        jget(req,"instance",inst_s,sizeof(inst_s));
    ...
    /* ── MST: set region ─────────────────────────────────────── */
        ...
        jget(req,"name",name,sizeof(name));
        jget(req,"revision",rev_s,sizeof(rev_s));
    ...
    /* ── build and show outgoing BPDU ────────────────────────── */
        ...
        char port_s[16]={0}; jget(req,"port",port_s,sizeof(port_s));
        stp_port_t *p=stp_port_find(br,port_s);
        if(!p){snprintf(resp,rsz,"{\"status\":\"error\",\"msg\":\"port not found\"}");return;}
