                        ...
                        const char    *path)
{
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        char type[20]={0};
        if (vr_json_get_str(line, "type", type, sizeof(type)) != 0) continue;

        /* ── FDB entry ──────────────────────────────────────── */
        if (strcmp(type, "fdb") == 0) {
            char mac_s[24]={0}, port[16]={0}, vlan_s[8]={0}, flags_s[12]={0};
            vr_json_get_str(line,"mac",  mac_s,  sizeof(mac_s));
            vr_json_get_str(line,"port", port,   sizeof(port));
            vr_json_get_str(line,"vlan", vlan_s, sizeof(vlan_s));
            vr_json_get_str(line,"flags",flags_s,sizeof(flags_s));
        ...
        /* ── VLAN entry ─────────────────────────────────────── */
            ...
            vr_json_get_str(line,"vid", vid_s,sizeof(vid_s));
        ...
        /* ── VLAN port config ───────────────────────────────── */
            ...
            vr_json_get_str(line,"port", port,  sizeof(port));
            vr_json_get_str(line,"mode", mode_s,sizeof(mode_s));
            vr_json_get_str(line,"pvid", pvid_s,sizeof(pvid_s));
            if (!port[0]) continue;
        ...
        /* ── STP config ─────────────────────────────────────── */
            ...
            vr_json_get_str(line,"priority",  prio_s, sizeof(prio_s));
            vr_json_get_str(line,"hello",     hello_s,sizeof(hello_s));
            vr_json_get_str(line,"max_age",   age_s,  sizeof(age_s));
            vr_json_get_str(line,"fwd_delay", delay_s,sizeof(delay_s));
            vr_json_get_str(line,"mode",      mode_s, sizeof(mode_s));
            vr_json_get_str(line,"mst_region",region, sizeof(region));
            vr_json_get_str(line,"mst_revision",rev_s,sizeof(rev_s));
        ...
        /* ── LACP LAG ───────────────────────────────────────── */
            ...
            vr_json_get_str(line,"lag",lag_s,sizeof(lag_s));
            vr_json_get_str(line,"key",key_s,sizeof(key_s));
            if (!lag_s[0]) continue;
