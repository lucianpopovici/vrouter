...
static void vlan_handler(void *ctx, const char *req,
    ...
    if(strcmp(cmd,"add")==0){
        ...
        vr_json_get_str(req,"vlan",vs,sizeof(vs)); vr_json_get_str(req,"name",name,sizeof(name));
        uint16_t vid=(uint16_t)atoi(vs);
        int rc=vlan_add(db,vid,name);
        snprintf(resp,rsz,"{\"status\":\"%s\",\"vlan\":%u}",rc==0?"ok":"error",vid);

    } else if(strcmp(cmd,"del")==0){
        char vs[8]={0}; vr_json_get_str(req,"vlan",vs,sizeof(vs));
        ...
        vr_json_get_str(req,"port",port,sizeof(port));
        vr_json_get_str(req,"mode",mode_s,sizeof(mode_s));
        vr_json_get_str(req,"pvid",pvid_s,sizeof(pvid_s));
        ...
        vr_json_get_str(req,"port",port,sizeof(port));
        vr_json_get_str(req,"vlan_lo",lo_s,sizeof(lo_s));
        vr_json_get_str(req,"vlan_hi",hi_s,sizeof(hi_s));
...
static void portsec_handler(void *ctx, const char *req,
    ...
    if(strcmp(cmd,"configure")==0){
        ...
        vr_json_get_str(req,"port",port,sizeof(port));
        vr_json_get_str(req,"max_macs",max_s,sizeof(max_s));
        vr_json_get_str(req,"violation",viol_s,sizeof(viol_s));
        ...
        vr_json_get_str(req,"port",port,sizeof(port));
        vr_json_get_str(req,"mac",mac_s,sizeof(mac_s));
        ...
        char port[16]={0}; vr_json_get_str(req,"port",port,sizeof(port));
...
static void storm_handler(void *ctx, const char *req,
    ...
    if(strcmp(cmd,"set_rate")==0){
        ...
        vr_json_get_str(req,"port",port,sizeof(port));
        vr_json_get_str(req,"type",type_s,sizeof(type_s));
        vr_json_get_str(req,"pps",pps_s,sizeof(pps_s));
        vr_json_get_str(req,"burst",burst_s,sizeof(burst_s));
...
static void igmp_handler(void *ctx, const char *req,
    ...
    if(strcmp(cmd,"report")==0||strcmp(cmd,"leave")==0){
        ...
        vr_json_get_str(req,"port",port,sizeof(port));
        vr_json_get_str(req,"group",grp_s,sizeof(grp_s));
        vr_json_get_str(req,"vlan",vlan_s,sizeof(vlan_s));
...
static void arp_handler(void *ctx, const char *req,
    ...
    if(strcmp(cmd,"learn")==0){
        ...
        vr_json_get_str(req,"mac",mac_s,sizeof(mac_s));
        vr_json_get_str(req,"ip",ip_s,sizeof(ip_s));
        vr_json_get_str(req,"port",port,sizeof(port));
        vr_json_get_str(req,"vlan",vlan_s,sizeof(vlan_s));
        vr_json_get_str(req,"type",type_s,sizeof(type_s));
        vr_json_get_str(req,"ipv6",v6_s,sizeof(v6_s));
        uint8_t mac[6]; fdb_mac_parse(mac_s,mac);
        uint8_t ip[16]={0};
...
static void lacp_handler(void *ctx, const char *req,
    ...
    if(strcmp(cmd,"lag_add")==0){
        ...
        vr_json_get_str(req,"lag",name,sizeof(name));
        vr_json_get_str(req,"key",key_s,sizeof(key_s));
        ...
        char name[16]={0}; vr_json_get_str(req,"lag",name,sizeof(name));
        int rc=lacp_lag_del(lt,name);
        snprintf(resp,rsz,"{\"status\":\"%s\"}",rc==0?"ok":"not_found");

    } else if(strcmp(cmd,"member_add")==0){
        char lag_s[16]={0},port[16]={0},mode_s[12]={0};
        vr_json_get_str(req,"lag",lag_s,sizeof(lag_s));
        vr_json_get_str(req,"port",port,sizeof(port));
        vr_json_get_str(req,"mode",mode_s,sizeof(mode_s));
        ...
        vr_json_get_str(req,"lag",lag_s,sizeof(lag_s));
        vr_json_get_str(req,"port",port,sizeof(port));
        vr_json_get_str(req,"actor_sys",sys_s,sizeof(sys_s));
        vr_json_get_str(req,"actor_key",key_s,sizeof(key_s));
        vr_json_get_str(req,"actor_state",state_s,sizeof(state_s));
        lacp_lag_t *l=lacp_lag_find(lt,lag_s);
        ...
        vr_json_get_str(req,"lag",lag_s,sizeof(lag_s));
        vr_json_get_str(req,"src",src_s,sizeof(src_s));
        vr_json_get_str(req,"dst",dst_s,sizeof(dst_s));
        lacp_lag_t *l=lacp_lag_find(lt,lag_s);
        if(!l){snprintf(resp,rsz,"{\"status\":\"error\",\"msg\":\"lag not found\"}");return;}
