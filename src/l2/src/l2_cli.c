...
int l2_cli_load_config(l2_config_t *cfg)
{
    ...
    if (vr_json_get_str(buf, "FDB_AGE_SEC",    val, sizeof(val)) == 0) {
        int v = atoi(val);
        if (v >= 10 && v <= 86400) cfg->fdb_age_sec = (uint32_t)v;
    }
    if (vr_json_get_str(buf, "FDB_MAX_ENTRIES", val, sizeof(val)) == 0) {
        int v = atoi(val);
        if (v >= 100 && v <= 524288) cfg->fdb_max_entries = (uint32_t)v;
    }
    if (vr_json_get_str(buf, "STP_MODE",        val, sizeof(val)) == 0) {
        /* vr_json_get_str already null-terminated val; clamp to field width */
        strncpy(cfg->stp_mode, val, sizeof(cfg->stp_mode));
        cfg->stp_mode[sizeof(cfg->stp_mode)-1] = '\0';
    }
    if (vr_json_get_str(buf, "STP_PRIORITY",    val, sizeof(val)) == 0) {
        int v = atoi(val);
        if (v >= 0 && v <= 61440 && v % 4096 == 0)
            cfg->stp_priority = (uint16_t)v;
    }
    if (vr_json_get_str(buf, "STP_HELLO",       val, sizeof(val)) == 0) {
        int v = atoi(val);
        if (v >= 1 && v <= 10) cfg->stp_hello = (uint16_t)v;
    }
    if (vr_json_get_str(buf, "STP_MAX_AGE",     val, sizeof(val)) == 0) {
        int v = atoi(val);
        if (v >= 6 && v <= 40) cfg->stp_max_age = (uint16_t)v;
    }
    if (vr_json_get_str(buf, "STP_FWD_DELAY",   val, sizeof(val)) == 0) {
        int v = atoi(val);
        if (v >= 4 && v <= 30) cfg->stp_fwd_delay = (uint16_t)v;
    }
    if (vr_json_get_str(buf, "MST_REGION",      val, sizeof(val)) == 0) {
        strncpy(cfg->mst_region, val, sizeof(cfg->mst_region));
        cfg->mst_region[sizeof(cfg->mst_region)-1] = '\0';
    }
    if (vr_json_get_str(buf, "MST_REVISION",    val, sizeof(val)) == 0) {
        int v = atoi(val);
        if (v >= 0 && v <= 65535) cfg->mst_revision = (uint32_t)v;
    }

    if (vr_json_get_str(buf, "SOCK_DIR", val, sizeof(val)) == 0 && val[0])
