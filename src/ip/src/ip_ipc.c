...
static char *handle_add_addr(ip_ctx_t *ctx, const char *req)
{
    ...
    long ifindex = vr_json_get_int(req,"ifindex",-1);
    if (ifindex<0 || vr_json_get_str(req,"prefix",pfx_str,sizeof(pfx_str))!=0)
        return err_json("missing ifindex or prefix");
    ip_prefix_t pfx;
    if (ip_prefix_parse(pfx_str,&pfx)!=IP_OK) return err_json("invalid prefix");
    uint32_t vl=(uint32_t)vr_json_get_int(req,"valid_lft",-1);
    uint32_t pl=(uint32_t)vr_json_get_int(req,"pref_lft",-1);
...
{
    long filter = vr_json_get_int(req,"ifindex",-1);
...
static bool parse_nh(const char *req, ip_addr_t *nh, uint32_t *oif)
{
    ...
    if (vr_json_get_str(req,"nexthop",nhstr,sizeof(nhstr))!=0) return false;
    if (ip_addr_parse(nhstr,nh)!=IP_OK) return false;
    long idx = vr_json_get_int(req,"ifindex",-1);
...
{
    char pfx_str[INET6_ADDRSTRLEN+4]={0};
    if (vr_json_get_str(req,"prefix",pfx_str,sizeof(pfx_str))!=0)
    ...
    long     ad_l   = vr_json_get_int(req,"ad",-1);
    uint8_t  ad     = (uint8_t)(ad_l>=0 ? ad_l : IP_AD_STATIC);
    long     ml     = vr_json_get_int(req,"metric",-1);
    uint32_t metric = ml>=0 ? (uint32_t)ml : 0;
    long     wl     = vr_json_get_int(req,"weight",-1);
...
{
    char af_str[8]={0};
    vr_json_get_str(req,"af",af_str,sizeof(af_str));
...
static char *handle_lookup(ip_ctx_t *ctx, const char *req)
{
    ...
    if (vr_json_get_str(req,"dst",dst_str,sizeof(dst_str))!=0)
    ...
    if (vr_json_get_str(req,"src",src_str,sizeof(src_str))==0 &&
        ip_addr_parse(src_str,&flow.src)==IP_OK) {
        flow.dst = dst;
        long sp=vr_json_get_int(req,"src_port",-1); if (sp>=0) flow.src_port=(uint16_t)sp;
        long dp=vr_json_get_int(req,"dst_port",-1); if (dp>=0) flow.dst_port=(uint16_t)dp;
        long pr=vr_json_get_int(req,"proto",-1);    if (pr>=0) flow.proto   =(uint8_t)pr;
        has_flow=true;
    }
...
static char *handle_set_ecmp_hash(ip_ctx_t *ctx, const char *req)
{
    ...
    char pfx_str[INET6_ADDRSTRLEN+4]={0};
    long mode_l = vr_json_get_int(req,"mode",-1);
...
static char *handle_set_forwarding(ip_ctx_t *ctx, const char *req)
{
    ...
    ip_set_forwarding(ctx,af,vr_json_get_bool(req,"enable"));
    return ok_json();
}
...
char *ip_ipc_handle(ip_ctx_t *ctx, const char *req, size_t req_len)
{
    ...
    if (!strcmp(cmd,IP_CMD_CLEAR_STATS)) {
        char af_str[8]={0}; vr_json_get_str(req,"af",af_str,sizeof(af_str));
        ip_stats_clear(ctx, strcmp(af_str,"ipv6")==0?AF_INET6:AF_INET);
        return ok_json();
    }
    if (!strcmp(cmd,IP_CMD_SET_IF_FWD)) {
        long idx=vr_json_get_int(req,"ifindex",-1); if (idx<0) return err_json("missing ifindex");
        char af_str[8]={0}; vr_json_get_str(req,"af",af_str,sizeof(af_str));
        sa_family_t af=strcmp(af_str,"ipv6")==0?AF_INET6:AF_INET;
        int rc=ip_set_if_forwarding(ctx,(uint32_t)idx,af,vr_json_get_bool(req,"enable"));
        return rc==IP_OK ? ok_json() : err_json("interface not found");
    }
    if (!strcmp(cmd,IP_CMD_DUMP_CONFIG)) {
        char path[256]="ip_runtime_config.json";
        vr_json_get_str(req,"path",path,sizeof(path));
        char *buf=malloc(512);
    ...
    if (!strcmp(cmd,"get")) {
        char key[64]={0}; vr_json_get_str(req,"key",key,sizeof(key));
        bool v4=ip_get_forwarding(ctx,AF_INET), v6=ip_get_forwarding(ctx,AF_INET6);
        pthread_rwlock_rdlock(&ctx->cfg.lock);
    ...
    if (!strcmp(cmd,"set")) {
        ...
        vr_json_get_str(req,"key",key,sizeof(key)); vr_json_get_str(req,"value",val,sizeof(val));
        if (!strcmp(key,"IPV4_FWD")) {
