...
/* -----------------------------------------------------------------------
 * FNV-1a helpers (using shared vr_fnv1a from <vrouter/hash.h>)
 * --------------------------------------------------------------------- */
...
{
    if (addr->af == AF_INET)
        return vr_fnv1a_mod((const uint8_t *)&addr->u.v4, 4, n_buckets);
    return vr_fnv1a_mod((const uint8_t *)&addr->u.v6, 16, n_buckets);
}
...
uint32_t ip_fnv1a_prefix(const ip_prefix_t *pfx, uint32_t n_buckets)
{
    ...
    buf[alen] = pfx->plen;
    return vr_fnv1a_mod(buf, alen + 1, n_buckets);
}
...
#define MIX(data, len) do { \
    for (size_t _i = 0; _i < (len); _i++) { \
        h ^= ((const uint8_t *)(data))[_i]; h *= VR_FNV_PRIME; \
    } \
} while(0)
...
{
    return vr_fnv1a_mod((const uint8_t *)&ifindex, sizeof(ifindex), t->n_buckets);
}
...
int ip_load_config(ip_ctx_t *ctx, const char *path)
{
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (strcmp(type_buf, "fwd_config") == 0) {
            pthread_rwlock_wrlock(&ctx->cfg.lock);
            ctx->cfg.ipv4_forwarding  = vr_json_get_bool(line, "ipv4_fwd");
            ctx->cfg.ipv6_forwarding  = vr_json_get_bool(line, "ipv6_fwd");
            ctx->cfg.flags            = (uint32_t)vr_json_get_int(line, "flags", -1);
            long ttl = vr_json_get_int(line, "default_ttl", -1);
            ctx->cfg.default_ttl      = ttl > 0 ? (uint8_t)ttl : 64;
            long hl  = vr_json_get_int(line, "default_hop_limit", -1);
            ctx->cfg.default_hop_limit = hl > 0 ? (uint8_t)hl : 64;
            long hm  = vr_json_get_int(line, "ecmp_hash_mode", -1);
            ...
            vr_json_get_str(line,"name",ifname,sizeof(ifname));
            vr_json_get_str(line,"mac", mac_str,sizeof(mac_str));
            uint32_t ifindex=(uint32_t)vr_json_get_int(line,"ifindex",-1);
            uint32_t flags  =(uint32_t)vr_json_get_int(line,"flags",-1);
            uint32_t mtu    =(uint32_t)vr_json_get_int(line,"mtu",-1);
            uint8_t mac[6]={0};
            sscanf(mac_str,"%hhx:%hhx:%hhx:%hhx:%hhx:%hhx",
                   &mac[0],&mac[1],&mac[2],&mac[3],&mac[4],&mac[5]);
            ip_if_add(ctx, ifname, ifindex, flags, mtu, mac);
            ip_set_if_forwarding(ctx,ifindex,AF_INET, vr_json_get_bool(line,"ip4_fwd"));
            ip_set_if_forwarding(ctx,ifindex,AF_INET6,vr_json_get_bool(line,"ip6_fwd"));

        } else if (strcmp(type_buf, "address") == 0) {
            uint32_t ifindex=(uint32_t)vr_json_get_int(line,"ifindex",-1);
            vr_json_get_str(line,"prefix",str_buf,sizeof(str_buf));
            ip_prefix_t pfx;
            if (ip_prefix_parse(str_buf,&pfx)==IP_OK) {
                uint32_t vl=(uint32_t)vr_json_get_int(line,"valid_lft",-1);
                uint32_t pl=(uint32_t)vr_json_get_int(line,"pref_lft",-1);
            ...
            uint8_t  ad     = (uint8_t) vr_json_get_int(line,"ad",-1);
            uint32_t metric = (uint32_t)vr_json_get_int(line,"metric",-1);
            uint32_t hmode  = (uint32_t)vr_json_get_int(line,"ecmp_hash_mode",-1);
            long     np_l   = vr_json_get_int(line,"n_paths",-1);
            ...
            for (uint8_t p = 0; p < n_paths; p++) {
                ...
                uint32_t oif    = (uint32_t)vr_json_get_int(line, ifk, -1);
                long     wl     = vr_json_get_int(line, wk, -1);
