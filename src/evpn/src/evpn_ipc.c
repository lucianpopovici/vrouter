...
static bool parse_addr(const char *req, const char *key, evpn_addr_t *out)
{
    ...
    if (vr_json_get_str(req, key, s, sizeof(s)) != 0) return false;
...
{
    long long eid = jll(req, "evi_id"); if (eid <= 0) return err("missing evi_id");
    char rd_s[32] = {0}; vr_json_get_str(req, "rd", rd_s, sizeof(rd_s));
...
{
    long long eid = jll(req, "evi_id"); if (eid <= 0) return err("missing evi_id");
    char rt_s[32] = {0}; vr_json_get_str(req, "rt", rt_s, sizeof(rt_s));
    uint64_t rt = parse_rd_str(rt_s); if (!rt) return err("invalid rt");
    bool is_exp = vr_json_get_bool(req, "export");
    bool is_imp = vr_json_get_bool(req, "import");
...
static char *h_add_prefix(evpn_ctx_t *ctx, const char *req)
{
    ...
    if (vr_json_get_str(req, "prefix", pfx_s, sizeof(pfx_s)) != 0) return err("missing prefix");
    evpn_prefix_t pfx;
    ...
    evpn_addr_t vtep = {0}; parse_addr(req, "vtep", &vtep);
    char rd_s[32] = {0}; vr_json_get_str(req, "rd", rd_s, sizeof(rd_s));
    bool local = vr_json_get_bool(req, "local");
...
{
    char esi_s[40] = {0}; if (vr_json_get_str(req, "esi", esi_s, sizeof(esi_s)) != 0) return err("missing esi");
...
{
    (void)req_len;
    char cmd[64] = {0}; vr_json_get_str(req, "cmd", cmd, sizeof(cmd));
