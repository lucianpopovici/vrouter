...
static char *h_create(vrf_ctx_t *ctx,const char *req){
    ...
    if (vr_json_get_str(req,"name",name,sizeof(name))!=0) return err_json("missing name");
...
static char *h_get(vrf_ctx_t *ctx,const char *req){
    ...
    else if (vr_json_get_str(req,"name",nb,sizeof(nb))==0) v=vrf_find_by_name(ctx,nb);
...
static char *h_bind_if(vrf_ctx_t *ctx,const char *req){
    ...
    char ifname[IFNAMSIZ]={0}; vr_json_get_str(req,"ifname",ifname,sizeof(ifname));
...
static char *h_add_route(vrf_ctx_t *ctx,const char *req){
    ...
    if (vid<0||vr_json_get_str(req,"prefix",ps,sizeof(ps))!=0) return err_json("missing vrf_id or prefix");
...
static char *h_list_routes(vrf_ctx_t *ctx,const char *req){
    ...
    char afs[8]={0}; vr_json_get_str(req,"af",afs,sizeof(afs));
...
static char *h_lookup(vrf_ctx_t *ctx,const char *req){
    ...
    long long vid=jll(req,"vrf_id");
    if (vid<0||vr_json_get_str(req,"dst",ds,sizeof(ds))!=0) return err_json("missing vrf_id or dst");
    ...
    if (vr_json_get_str(req,"src",ss,sizeof(ss))==0&&vrf_addr_parse(ss,&flow.src)==VRF_OK) {
        flow.dst=dst;
...
static char *h_set_ecmp_hash(vrf_ctx_t *ctx,const char *req){
    ...
    char ps[INET6_ADDRSTRLEN+4]={0};
    if (vid>=0&&vr_json_get_str(req,"prefix",ps,sizeof(ps))==0) {
        vrf_prefix_t pfx; if (vrf_prefix_parse(ps,&pfx)!=VRF_OK) return err_json("invalid prefix");
...
static char *h_leak(vrf_ctx_t *ctx,const char *req){
    ...
    if (src<0||dst<0||vr_json_get_str(req,"prefix",ps,sizeof(ps))!=0) return err_json("missing fields");
    vrf_prefix_t pfx; if (vrf_prefix_parse(ps,&pfx)!=VRF_OK) return err_json("invalid prefix");
    vrf_addr_t nh={0}; uint32_t oif=0;
    if (!parse_nh_vrf(req,&nh,&oif)){nh.af=pfx.addr.af;long long ii=jll(req,"ifindex");if(ii>=0)oif=(uint32_t)ii;}
    long long ml=jll(req,"metric"); uint32_t metric=ml>=0?(uint32_t)ml:0;
...
char *vrf_ipc_handle(vrf_ctx_t *ctx,const char *req,size_t req_len){
    (void)req_len; char cmd[64]={0}; vr_json_get_str(req,"cmd",cmd,sizeof(cmd));
    ...
    if (!strcmp(cmd,VRF_CMD_DUMP_CONFIG)){
        char path[256]="vrf_runtime_config.json"; vr_json_get_str(req,"path",path,sizeof(path));
        char *buf=malloc(512);
    ...
    if (!strcmp(cmd,"get")){
        char key[64]=""; vr_json_get_str(req,"key",key,sizeof(key));
