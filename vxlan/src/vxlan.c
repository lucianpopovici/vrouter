...
static uint16_t entropy_port(const uint8_t *inner, size_t inner_len)
{
    ...
    uint32_t h = fnv1a(inner, inner_len < 32 ? inner_len : 32, 0);
                             ...
                             uint32_t n)
{
    ...
    buf[7] = (vni >>  8) & 0xff;
    buf[8] =  vni        & 0xff;
    return fnv1a(buf, 9, n);
}

static uint32_t vni_hash(uint32_t vni, uint32_t n)
{
    return fnv1a((const uint8_t *)&vni, 4, n);
}
...
int vxlan_load_config(vxlan_ctx_t *ctx, const char *path)
{
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (!strcmp(type_buf, "global")) {
            ...
            jstr(line, "local_ip", ip_s, sizeof(ip_s));
            ...
            jstr(line, "mcast_group", mcast_s, sizeof(mcast_s));
            if (mcast_s[0]) {
            ...
            jstr(line, "remote", rip_s, sizeof(rip_s));
            vxlan_addr_t rip;
            ...
            jstr(line, "remote_ip", rip_s, sizeof(rip_s));
