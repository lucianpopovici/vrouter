...
static uint32_t fnv1a(const uint8_t *d, size_t l, uint32_t n)
{
    uint32_t h = FNV_OFFSET;
    for (size_t i = 0; i < l; i++) { h ^= d[i]; h *= FNV_PRIME; }
    return h % n;
}

uint32_t evpn_fnv1a_mac(const evpn_mac_t *mac, uint32_t n)
{ return fnv1a(mac->b, 6, n); }

uint32_t evpn_fnv1a_addr(const evpn_addr_t *a, uint32_t n)
{
    if (a->af == AF_INET)  return fnv1a((const uint8_t *)&a->u.v4,  4, n);
    if (a->af == AF_INET6) return fnv1a((const uint8_t *)&a->u.v6, 16, n);
    return 0;
}
...
{
    uint32_t h = FNV_OFFSET; h ^= id; h *= FNV_PRIME;
    return h % t->n_buckets;
}
...
static bool jbool(const char *l, const char *k, const char *val)
{
    char s[128]; snprintf(s, sizeof(s), "\"%s\":%s", k, val);
...
int evpn_load_config(evpn_ctx_t *ctx, const char *path)
{
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (strcmp(type_buf, "global") == 0) {
            ...
            jstr(line, "local_vtep", vtep_s, sizeof(vtep_s));
            ...
            jstr(line,"esi",esi_s,sizeof(esi_s));
            jstr(line,"sys_mac",mac_s,sizeof(mac_s));
            ...
            evpn_evi_create(ctx,(uint32_t)eid,l2,l3,vrf,
                            flg&~EVPN_EVI_ACTIVE);
            /* restore RD */
            char rd_s[32]={0}; jstr(line,"rd",rd_s,sizeof(rd_s));
            uint64_t rd = parse_rd_str(rd_s);
            if (rd) evpn_evi_set_rd(ctx,(uint32_t)eid,rd);
            /* restore IRB */
            char irb_ip_s[INET6_ADDRSTRLEN]={0}, irb_mac_s[20]={0};
            jstr(line,"irb_ip",irb_ip_s,sizeof(irb_ip_s));
            jstr(line,"irb_mac",irb_mac_s,sizeof(irb_mac_s));
            if (irb_ip_s[0]) {
            ...
            jstr(line,"rt",rt_s,sizeof(rt_s));
            jstr(line,"dir",dir,sizeof(dir));
            ...
            jstr(line,"vtep",vtep_s,sizeof(vtep_s));
            jstr(line,"rd",rd_s,sizeof(rd_s));
            evpn_addr_t vtep_ip;
            ...
            char vtep_s[INET6_ADDRSTRLEN]={0};
            jstr(line,"mac",mac_s,sizeof(mac_s));
            jstr(line,"ip",ip_s,sizeof(ip_s));
            jstr(line,"vtep",vtep_s,sizeof(vtep_s));
            ...
            char vtep_s[INET6_ADDRSTRLEN]={0};
            jstr(line,"prefix",pfx_s,sizeof(pfx_s));
            jstr(line,"gw_ip",gw_ip_s,sizeof(gw_ip_s));
            jstr(line,"gw_mac",gw_mac_s,sizeof(gw_mac_s));
            jstr(line,"vtep",vtep_s,sizeof(vtep_s));
            ...
            evpn_addr_t vtep_ip={0}; if (vtep_s[0]) evpn_addr_parse(vtep_s,&vtep_ip);
            long long rdl = jll(line,"rd");
            bool local = jbool(line,"local","true");
