...
#define IP_STATS_INTERVAL_SEC   60
...
/* Unified address: holds either an IPv4 or IPv6 address */
typedef struct ip_addr {
    sa_family_t     af;         /* AF_INET or AF_INET6 */
    union {
        struct in_addr  v4;
        struct in6_addr v6;
    } u;
...
/* Per-address entry on an interface */
typedef struct ip_if_addr {
    ip_prefix_t     prefix;     /* address/prefix-length */
    ip_addr_t       broadcast;  /* v4 only */
    ip_addr_t       anycast;    /* v6 only */
    uint32_t        flags;
    uint8_t         state;      /* preferred/deprecated/tentative (v6) */
    uint32_t        valid_lft;  /* v6: valid lifetime (secs, 0=forever) */
    uint32_t        pref_lft;   /* v6: preferred lifetime */
    time_t          assigned_at;
...
typedef struct ip_interface {
    char            name[IFNAMSIZ];
    uint32_t        ifindex;
    uint32_t        flags;          /* IP_IF_* */
    uint32_t        mtu;
    uint8_t         mac[6];
    bool            ip4_fwd;        /* per-interface IPv4 forwarding */
    bool            ip6_fwd;        /* per-interface IPv6 forwarding */
    bool            ip6_ra_enable;  /* send RA (router advertisements) */
    uint8_t         ip6_hop_limit;
    ip_if_addr_t   *addrs;          /* linked list of addresses */
    uint32_t        n_addrs;
    /* stats */
    uint64_t        rx_pkts;
    uint64_t        tx_pkts;
    ...
    struct ip_interface *next;      /* hash chain */
} ip_interface_t;

/* Forwarding cache entry (single resolved next-hop) */
typedef struct ip_fwd_entry {
    ip_prefix_t     prefix;
    ip_addr_t       nexthop;
    uint32_t        out_ifindex;
    uint8_t         ad;             /* administrative distance */
    uint32_t        metric;
    uint64_t        hit_count;
    time_t          installed_at;
    struct ip_fwd_entry *next;      /* hash chain */
} ip_fwd_entry_t;

/* Global forwarding table (separate v4 / v6) */
...
typedef struct ip_fwd_config {
    uint32_t    flags;              /* IP_FWD_* bitmask */
    bool        ipv4_forwarding;    /* global IPv4 forwarding kill-switch */
    bool        ipv6_forwarding;    /* global IPv6 forwarding kill-switch */
    uint8_t     default_ttl;        /* default TTL for originated packets */
    uint8_t     default_hop_limit;  /* default hop-limit for IPv6 */
    uint32_t    fwd_table_max;      /* max FIB entries */
    pthread_rwlock_t lock;
} ip_fwd_config_t;
...
typedef struct ip_stats {
    ...
    uint64_t    rx_bytes;
    uint64_t    tx_bytes;
    uint64_t    rx_drop_ttl;        /* TTL/HL expired */
    uint64_t    rx_drop_noroute;    /* no route to host */
    uint64_t    rx_drop_martian;    /* martian source */
    uint64_t    rx_drop_rpf;        /* RPF check failed */
    uint64_t    rx_errors;
    uint64_t    tx_errors;
...
typedef struct ip_ctx {
    ...
    ip_fwd_table_t  fwd4;           /* IPv4 forwarding table */
    ip_fwd_table_t  fwd6;           /* IPv6 forwarding table */
    ip_fwd_config_t cfg;
    ip_stats_t      stats4;
    ip_stats_t      stats6;
    char            sock_path[256];
...
int ip_addr_list(ip_ctx_t *ctx, uint32_t ifindex,
                 ip_if_addr_t **out, uint32_t *out_n);
...
int ip_fwd_list(ip_ctx_t *ctx, sa_family_t af,
                ip_fwd_entry_t **out, uint32_t *out_n);
